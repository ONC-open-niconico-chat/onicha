...
export default function PostDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleRepostToggle = async () => {
    if (!myId || !post) return;
    const nextIsReposted = !post.is_reposted_by_me;
    const nextCount = (post.retweets_count || 0) + (nextIsReposted ? 1 : -1);

    // 楽観的UI更新
    setPost(prev => prev ? {
      ...prev,
      is_reposted_by_me: nextIsReposted,
      retweets_count: nextCount
    } : null);
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex justify-between mt-3 max-w-xs text-gray-500">
              ...
              <button 
                ...
                className={`flex items-center gap-1 transition ${post.is_reposted_by_me ? 'text-green-500' : 'hover:text-green-500'}`}
              >
                <Repeat2 size={18} />
                <span className="text-xs">{(post.retweets_count ?? 0) > 0 ? post.retweets_count : ''}</span>
              </button>
